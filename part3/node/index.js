    ...
    validate: {
      validator: function(v) {
        ...
        for (let i = 0; i < v.length; i++) {
          ...
          } else if (Number(numDash) === 0 || Number(numDash) === 1) {
            if (v.charAt(i).isNAN) {
              return false
            }
          } else {
...
app.get('/info', (request, response) => {
  let numPeople = phonebookEntry.count({})
  response.send(`Phonebook has info for ${numPeople} people<br>${new Date()}`)
})

app.delete('/api/persons/:id', (request, response, next) => {
  phonebookEntry.findByIdAndRemove(request.params.id)
    .then(response.status(204).end()).catch(error => next(error))
})
